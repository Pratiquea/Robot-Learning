...
from network import ConnectNet, AlphaLoss, board_data
...
def train(net, dataset, optimizer, scheduler, start_epoch, cpu, args, iteration):
    if not os.path.exists('./logz/'):
        print('\nCREATED LOG PATH \n')
        os.makedirs('./logz/')

    path = glob.glob(os.path.expanduser('./logz/'))[0]
    print("\n###############################\n {}\n###############################\n".format(path))
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        scheduler.step()
        writer.add_scalar('data/episode_loss', sum(losses_per_epoch), epoch)
